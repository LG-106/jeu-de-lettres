...
if "index" not in st.session_state: st.session_state.index = 0 st.session_state.score = 0 st.session_state.total = 0 st.session_state.clicked = [] st.session_state.locked = False

st.set_page_config(page_title="Jeu des Lettres", layout="wide") st.title("🎯 Jeu des Lettres")

Choix du mode utilisateur

mode = st.radio("Qui utilise cette application ?", ["Elève", "Enseignant 🔐"], horizontal=True)

if mode == "Enseignant 🔐": mdp = st.text_input("Mot de passe enseignant :", type="password") if mdp != "modou123": st.warning("Mot de passe incorrect.") st.stop() show_teacher = True else: show_teacher = False

Saisie enseignant (protégée)

if show_teacher: with st.expander("🧑‍🏫 Paramètres enseignant", expanded=True): phrases_input = st.text_area("Phrases (une par ligne)", height=150) lettres_input = st.text_area("Lettres à chercher (une ligne par phrase, ou vide pour répéter)", height=150) if st.button("Lancer le jeu"): st.session_state.index = 0 st.session_state.score = 0 st.session_state.total = 0 st.session_state.locked = False
...
if "phrases" in st.session_state and st.session_state.index < len(st.session_state.phrases): phrase = st.session_state.phrases[st.session_state.index] lettres_cibles = st.session_state.consignes[st.session_state.index]
...
for i, lettre in enumerate(phrase):
    ...
    if st.session_state.locked:
        ...
        color = "#00cc44" if st.session_state.clicked[i] else "#ffffff"
